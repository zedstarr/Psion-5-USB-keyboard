/*
  ...
  Based on https://github.com/RasmusB/USB-Keyboard-Adapter
    ...
    for ( int col = 0; col < NCOLS; col++ ) {
      ...
      if (keyScancode[row][col] != 0) {
        ...
        if ( pressedArray[row][col] > previousArray[row][col] ) {
          ...
          // Ugly hack to fix the Fn Key 
          // Is the Fn key pressed?
          if ( pressedArray[6][9] > 0 ) {

             Keyboard.press(fnSpecialKeys[row][col]); // This is a special "psion" key
             if ( pressedArray[5][0] > 0 ) { // CAPS LOCK
              CapsLock = !CapsLock;
             } 
